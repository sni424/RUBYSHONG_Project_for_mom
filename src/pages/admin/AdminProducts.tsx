...
import {
  deleteProduct,
  getProductDeleteLogs,
  getProducts,
  type ProductSearchParams,
} from '@/api/productApi';
import type { Product, ProductDeleteLog } from '@/constants/type';
...
const AdminProducts = () => {
  ...
  // 현재 보고 있는 탭
  const [activeTab, setActiveTab] = useState<'products' | 'deleteLogs'>('products');

  // 상품 삭제 이력 목록
  const [productDeleteLogs, setProductDeleteLogs] = useState<ProductDeleteLog[]>([]);

  // 상품 삭제 이력 로딩 상태
  const [isDeleteLogLoading, setIsDeleteLogLoading] = useState(false);
  ...
  // 상품 삭제 이력 조회
  const fetchProductDeleteLogs = async () => {
    try {
      setIsDeleteLogLoading(true);
      setErrorMessage('');

      // 상품 삭제 이력 API 요청
      const data = await getProductDeleteLogs();
        ...
        <div className="flex gap-6 overflow-x-auto text-sm font-semibold">
          {[
            ...
            <button
              ...
              onClick={() => {
                setErrorMessage('');
                setActiveTab(tab.value as 'products' | 'deleteLogs');

                // 삭제 이력 탭을 누르면 삭제 이력 조회
                if (tab.value === 'deleteLogs') {
                  fetchProductDeleteLogs();
                }
                if (tab.value === 'deleteLogs') {
                  fetchProductDeleteLogs();
                }
          ...
          <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-180 text-left text-sm">
                <thead className="bg-slate-50 text-slate-600">
                  <tr>
                    <th className="px-4 py-3">삭제 상품</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {productDeleteLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50/70">
                      <td className="px-4 py-3 font-semibold text-slate-900">{log.productName}</td>
                      <td className="px-4 py-3 text-slate-500">{log.productId}</td>
                      <td className="px-4 py-3 text-slate-500">{log.productSlug}</td>
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800">{log.deletedBy.email}</p>
                        <p className="text-xs text-slate-500">{log.deletedBy.role}</p>
                      </td>
                      <td className="px-4 py-3 text-slate-500">
              ...
              {!isDeleteLogLoading && productDeleteLogs.length === 0 && (
                <div className="border-t border-slate-100 px-4 py-12 text-center text-sm font-semibold text-slate-500">
                  삭제 이력이 없습니다.
                </div>
              )}
